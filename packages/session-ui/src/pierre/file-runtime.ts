...
function scheduleReady(opts: ReadyOptions, token: number, settle: number) {
  const step = (left: number) => {
    if (token !== opts.state.token) return
...
function watchRoot(
  ...
  if (opts.isReady(root)) {
    scheduleReady(opts, token, settle)
    return
  }
...
function watchForRoot(
  ...
  opts.state.observer = new MutationObserver(() => {
    ...
    watchRoot(opts, root, token, settle)
  })
...
export function notifyShadowReady(opts: ReadyOptions) {
  clearReadyWatcher(opts.state)
  opts.state.token += 1
  ...
  if (root) {
    ...
    watchForRoot(opts, token, settle)
  }
}